...
//   // Process the next pending call
//   const processNextCall = async () => {
//     if (pendingCalls.length === 0 || processingCall) return;
...
//       await initiateCall(call.phoneNumber);
      
//       // Update the call status in Firebase
//       const callRef = doc(db, 'callRequests', call.id);
...
//       // Remove the processed call from the list
//       setPendingCalls(pendingCalls.slice(1));
//     } catch (error) {
...
//       // Remove the failed call from the list
//       setPendingCalls(pendingCalls.slice(1));
//     } finally {
...
//   // Fetch pending calls on component mount and set up interval
//   useEffect(() => {
//     fetchPendingCalls();
...
//   // Process calls automatically when the component updates
//   useEffect(() => {
//     if (pendingCalls.length > 0 && !processingCall) {
//       processNextCall();
//     }
//   }, [pendingCalls, processingCall]);
...
const VoiceAssistant = () => {
  ...
  const processNextCall = async () => {
    ...
    try {
      console.log(`Processing call for number: ${call.phoneNumber}`);
      
      // Update status to processing
      const callRef = doc(db, 'callRequests', call.id);
      await updateDoc(callRef, {
        status: 'processing',
        processStartedAt: serverTimestamp()
      });
      
      // Direct API call to server instead of using twilioService
      const response = await axios.post('https://8e7e-14-139-125-231.ngrok-free.app/api/twilio/call', {
        phoneNumber: call.phoneNumber,
        userId: call.userId || 'guest'
      });
      ...
      console.error('Error processing call:', error);
      setError(`Call processing error: ${error.message}`);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log("Interval triggered - fetching pending calls");
      fetchPendingCalls();
    }, 10000); // Refresh every 10 seconds for faster response
  ...
  useEffect(() => {
    if (pendingCalls.length > 0 && !processingCall) {
      console.log(`Found ${pendingCalls.length} pending calls, processing next call`);
      processNextCall();
    }
  }, [pendingCalls, processingCall]);
